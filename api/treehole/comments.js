// GET  /api/treehole/comments?postId=xxx&userId=yyy&postAuthorId=zzz  —  读取评论
...
async function handleGet(req, res) {
  ...
  if (!postId) {
    sendJson(res, 400, { success: false, message: '缺少 postId 参数' });
    return;
  }

  const data = await callBitableApi(
    'GET',
    `tables/${TABLES.comments}/records?page_size=200`
  );
  ...
  const comments = (data.items || [])
    ...
    .map((r) => ({
      id: r.record_id,
      content: r.fields['评论内容'] || '',
      author: r.fields['评论者'] || '匿名用户',
      authorId: r.fields['评论者ID'] || '',
      commentType: Array.isArray(r.fields['评论类型'])
        ? r.fields['评论类型'][0]
        : r.fields['评论类型'] || '公开',
      timestamp: new Date(r.fields['评论时间'] || Date.now()).getTime(),
    }))
    .filter((c) => {
      // 公开评论所有人可见
      if (c.commentType === '公开') return true;
      // 悄悄话只有树洞发布者和评论者自己可见
      return authorId === currentUserId || c.authorId === currentUserId;
    })
...
async function handlePost(req, res) {
  ...
  if (!postId || !content) {
    sendJson(res, 400, { success: false, message: '缺少必要参数（postId / content）' });
    return;
  }
