...
async function handleGet(req, res) {
  const data = await callBitableApi('GET', `tables/${TABLES.posts}/records?page_size=100`);
  const records = data.items || [];
  const posts = records.map((r) => ({
    id: r.record_id, content: r.fields['内容'] || '', mood: r.fields['心情'] || '😊',
    author: r.fields['发布者'] || '匿名用户', authorId: r.fields['发布者ID'] || '',
    timestamp: r.fields['发布时间'] || Date.now(),
    date: new Date(r.fields['发布时间'] || Date.now()).toLocaleDateString('zh-CN'),
    like: r.fields['点赞数'] || 0, hug: r.fields['抱抱数'] || 0,
    same: r.fields['同感数'] || 0, inspired: r.fields['启发数'] || 0,
...
async function handlePost(req, res) {
  ...
  if (!content) { sendJson(res, 400, { success: false, message: '缺少 content 参数' }); return; }
  const now = Date.now();
  const fields = {
    '内容': content, '心情': mood || '😊', '发布者': userName || '匿名用户',
    '发布者ID': userId || 'anonymous', '发布时间': now,
    '点赞数': 0, '抱抱数': 0, '同感数': 0, '启发数': 0, '评论数': 0,
  };
